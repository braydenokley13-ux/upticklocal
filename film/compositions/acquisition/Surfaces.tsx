...
export function AcquisitionSurface({state,sourceId='car-wash'}: {state: AcquisitionSurface; sourceId?:string}) {
  useFilmFonts();
  const frame = useCurrentFrame();
  const source=A.sources.find(item=>item.id===sourceId);
  if(!source) throw new Error(`Unknown acquisition source: ${sourceId}`);
  if (state === 'placement') return <AbsoluteFill style={{background:C.marine, color:C.canvas, fontFamily:FONT.sans, padding:88, boxSizing:'border-box'}}>
    <div style={{fontSize:30, letterSpacing:4}}>UPTICK GROWTH · LOCAL PARTNER NETWORK</div>
    ...
    <div style={{display:'flex', alignItems:'end', justifyContent:'space-between', marginTop:60}}>
      <div style={{fontSize:76, letterSpacing:-2}}>{source.distanceMiles} mi <span style={{fontSize:40}}>away</span></div>
      <QrMark size={104}/>
    </div>
  </AbsoluteFill>;
  ...
  const title = state === 'permission' ? (frame<24 ? A.acquisitionOffer.redeemed : accepted ? A.permission.accepted : A.permission.headline)
    : isFriday ? (paid ? A.returnOffer.returned : 'Breakfast sandwich\n+ coffee')
    : state === 'redeemed' ? A.acquisitionOffer.redeemed : 'Your coffee\npass.';
  return <AbsoluteFill style={{background:C.canvas, color:C.ink, fontFamily:FONT.sans, padding:62, boxSizing:'border-box'}}>
    ...
    <div style={{fontSize:34, marginTop:100}}>{A.merchant}</div>
    <div style={{fontSize:24,marginTop:18,color:C.inkSoft}}>FROM {source.name.toUpperCase()}</div>
    ...
    <div style={{fontSize:36, lineHeight:1.4, marginTop:45, color:C.inkSoft}}>
      {state === 'permission' ? (frame<24 ? 'Your first coffee. Your choice what comes next.' : accepted ? 'Joe’s Friday offer. Only because you said yes.' : A.permission.detail)
    ...
    <div style={{position:'absolute', left:62, right:62, bottom:230, background:done?C.mintDeep:C.ink, color:C.canvas, padding:'36px 28px', fontSize:37, textAlign:'center', borderRadius:6,
      ...
      {state === 'permission' ? (accepted ? '✓  You’re in.' : A.permission.action)
        : state === 'redeemed' ? '✓  Coffee redeemed' : paid ? '✓  Paid at Joe’s' : isFriday ? 'Friday · 6–10 AM' : 'Ready at Joe’s'}
    </div>

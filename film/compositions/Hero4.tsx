...
const S = 0.62;
const BOX = { x: 1230, y: 112, w: PLAN_W * S, h: PLAN_H * S };
const ROT = -12;
...
export const Hero4 = () => {
  ...
  const sheetOpacity = 0.92 * sheetUp * (1 - sheetSink);
  const sheetBlur = 4 - 1.6 * sheetUp + 5 * sheetSink;
  ...
  return (
    <Frame bg="#040c10">
        ...
        <>
          <div style={{ position: "absolute", left: 0, top: 0, width: BOX.w, height: BOX.h, transform: sheetMatrix, transformOrigin: "0 0", opacity: sheetOpacity, filter: `blur(${sheetBlur.toFixed(1)}px)` }}>
            ...
            <div style={{ position: "absolute", left: 0, top: 0, width: PLAN_W, height: PLAN_H, transform: `scale(${S})`, transformOrigin: "0 0" }}>
              ...
              <div style={{ position: "absolute", left: PLAN_PAD + s[0], top: 60 + PLAN_ROWS.fuel + 14 + SHEET_PX * 1.18, width: s[5] - s[0], height: 2, background: COLOR.mint, opacity: lit * (1 - Math.min(1, anyLift * 3)) }} />
              <div style={{ position: "absolute", left: PLAN_PAD, top: 60 + PLAN_ROWS.fuel + 14, opacity: Math.min(1, anyLift * 3), fontFamily: FONT.sans, fontWeight: 300, fontSize: SHEET_PX, color: COLOR.onMarineSoft, whiteSpace: "nowrap" }}>
                Fuel:
                <span style={{ display: "inline-block", width: s[5] - s[0] - 8, height: 2, background: COLOR.mint, marginLeft: 8, verticalAlign: "middle", opacity: 0.7 }} />
              </div>
            </div>
          </div>
      ...
      <div style={{ position: "absolute", right: 1920 - 1180, top: 424, opacity: questionIn, transform: `translateX(${(1 - questionIn) * 40}px)`, textAlign: "right" }}>
      ...
      {[w1, w2, w3].map((word, i) => {
        ...
        const head = 0.45 * Math.min(1, t * 6) * (since > 0 ? 1 - since / 8 : 1);
        const segs: { d: string; o: number }[] = [];
        const L = 0.3;
        for (let k = 0; k < 10; k++) {
          const t0 = Math.max(0, t - L + (L * k) / 10);
          const t1 = Math.max(0, t - L + (L * (k + 1)) / 10);
      ...
      <div style={{ position: "absolute", left: ANSWER.x, bottom: 150, opacity: line1, fontFamily: FONT.sans, fontSize: 28, color: COLOR.onMarineSoft, letterSpacing: "-0.01em" }}>Already in what Joe approved, so Uptick answered.</div>
      <div style={{ position: "absolute", left: ANSWER.x, bottom: 108, opacity: line2, fontFamily: FONT.sans, fontSize: 24, color: COLOR.onMarineFaint, letterSpacing: "-0.01em" }}>Anything else goes to Joe.</div>

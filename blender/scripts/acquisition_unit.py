...
def next_stop_unit(x,y,source='car-wash',yaw=0):
    ...
    # Centred, this sat directly behind the spine, which is 1.35 tall and .26
    # deep: at 1920x1080 the operator signature read "BY    CK". It moves off
    # the post to the panel's lower right, where a maker's plate would be.
    part(B.text('next_stop_operator','BY UPTICK',.080,(.70,-.127,1.29),cream,extrude=.001))
    for dx in (-.52,.52):

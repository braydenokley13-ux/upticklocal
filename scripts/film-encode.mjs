 ...
 *   node scripts/film-encode.mjs hero1 proxy      # …/plates/proxy/seq/hero1/*.png       → …/plates/proxy/hero1.mp4
 *
...
const lane = process.argv[3] === "proxy" ? "proxy" : "final";
if (!shot) throw new Error("usage: node scripts/film-encode.mjs <shot> [proxy]");
const root = lane === "proxy" ? "public/film-rd/plates/proxy" : "public/film-rd/plates";
const seq = join(root, "seq", shot);
const frames = readdirSync(seq).filter((f) => f.endsWith(".png")).sort();
if (frames.length === 0) throw new Error(`no frames in ${seq}`);
const out = join(root, `${shot}.mp4`);
const args = ["ffmpeg", "-y", "-framerate", "24", "-i", join(seq, "%04d.png"), "-c:v", "libx264", "-preset", lane === "proxy" ? "veryfast" : "slow", "-crf", lane === "proxy" ? "20" : "15", "-pix_fmt", "yuv420p", "-g", "12", "-movflags", "+faststart", out];

...
# One worker by default and well back in the queue: while plates are rendering they are the
# critical path and this must not slow them. With the queue paused, CONC=4 uses the whole box.
nice -n 15 npx remotion render film/index.ts Film "$OUT/$name.mp4" --frames="$a-$b" --codec=h264 --crf=16 --concurrency="${CONC:-1}" --log=error
